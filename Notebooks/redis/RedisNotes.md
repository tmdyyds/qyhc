...
**加锁** : 加锁操作setnx或者set+lua(释放锁时)**加锁导致并发降低且加锁业务逻辑复杂**
    - 单机锁
        + 单机加锁直接变量0 1表示加锁即可, 
    - 分布式锁
        + 单机分布式锁,使用Redis单命令和LUA脚本
        + 基于多个Redis节点实现高可靠的分布式锁只能使用原子命令(单命令)或Redlock
            * Redlock加锁思路,客户端和多个独立的Redis实例依次请求加锁,如果客户端能够获取半数以上的实例成功的完成加锁,则获取锁成功。
            * Redlock加锁具体步骤
                - 客户端获取当前时间
                - 客户端依次像N个实例执行加锁操作
                - 客户端完成了和所有Redis实例加锁过程,客户端就需要计算整个加锁过程的总耗时(客户端加锁总耗时没有超过锁的有效时间)(满足加锁条件和计算总耗时后,重新计算锁的有效时间=锁的最初有效时间-客户端获取锁总耗时)
...
- DISCARD(discard)不是事务回滚,只是用来放弃事务执行
- WATCH 检测一个或多个键的值是否发生变化
- 关于 Redis 事务的使用，有几个细节
...
- 只读缓存,解决缓存和DB数据一致性问题
    ...
    + 删除和更新缓存问题与解决方法
        * **先删除缓存，再更新数据库**
            - 假设线程A先删缓存,没来及更新DB,B来就开始读取数据了,这时缓存没数据so直接读取DB数据然后更新缓存,等到线程 B 从数据库读取完数据、更新了缓存后，线程 A 才开始更新数据库，此时，缓存中的数据是旧值，而数据库中的是最新值，两者就不一致了
                + 在线程 A 更新完数据库值以后，我们可以让它先 sleep 一小段时间(这个时间不好确定)，再进行一次缓存删除操作(延迟删除,在更新DB时候执行延迟删除)
        * **先更新数据库值，再删除缓存值**
    ...
    + 建议优先使用先更新数据库再删除缓存的方法，（不过，当使用先更新数据库再删除缓存时，也有个地方需要注意，如果业务层要求必须读取一致的数据，那么，我们就需要在更新数据库时，先在 Redis 缓存客户端暂存并发读请求，等数据库更新完、缓存值删除后，再读取数据，从而保证数据一致性。）原因主要有两个
...
####如何解决缓存污染(访问次数很少的数据长期驻留在缓存中，就是缓存污染)问题？
- 
